...
export default function CustomerFloorplan(props) {
  ...
  const isFormValid = () => {
    if (!data.Exid.trim()) {
      alert("Please select exhibition name (Exid).");
      return false;
    }

    if (!data.Name_th.trim()) {
      alert("Please enter the Customer Name (Thai).");
      return false;
    }

    if (!data.Name_en.trim()) {
      alert("Please enter the Customer Name (English).");
      return false;
    }
  ...
  return (
    <section id="CustomerFloorplan">
      ...
      <div className="flex justify-between mt-4 w-full md:w-2/3">
        ...
        {isEditMode ? (
          <button
            className="px-2 bg-orange-500 hover:bg-orange-600 text-white rounded"
            onClick={editFloorplan}>
            Save Edit
          </button>
        ) : (
          <button className="px-6 btn-green" onClick={submitData}>
            Add
          </button>
        )}
      </div>
