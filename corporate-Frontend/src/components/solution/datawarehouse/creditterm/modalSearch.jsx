...
export default function ModalSeach(props) {
  ...
  const searchClick = async (name) => {
    if (name != "") {
      const res = await Axios.get(url + "/searchCustomer/" + name).then((res) => {
  ...
  const selectName = (id, fullName, searchName) => {
    props.fill(id, fullName, searchName);
  };
          ...
          <div className="relative bg-white rounded-lg shadow">
            ...
            <div className="p-6 space-y-6 max-h-[60vh] overflow-y-scroll">
              <ul>
                {customer.map((c) => (
                  <li
                    onClick={() => nameClick(c.id, c.name, customer)}
