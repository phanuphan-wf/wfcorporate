...
export default function Without_Zones({preview}) {
  ...
  const groupedBySale = useMemo(() => {
    ...
    return normalizedList.reduce((acc, row) => {
      const salesName = row.sales || "ไม่ระบุพนักงานขาย";

      if (!acc[salesName]) {
       acc[salesName] = [];
      }
      acc[salesName].push(row);
      return acc;
    }, {});
  }, [normalizedList]);

  // useEffect(() => {
  //   console.log(groupedBySale);
  // }, [groupedBySale]);
