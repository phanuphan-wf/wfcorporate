...
export default function ModalConfirmSave(props) {
  return (
    <div
    ...
    >
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 bg-white rounded-md border shadow-md px-4 py-3">
        ...
        <div className="p-4">
          <p>
            Please confirm to change payment term of{" "}
            <span className="font-semibold text-red-500">{props.name}</span>
          </p>
        </div>
        <div className="flex justify-end mb-4">
          <div className="flex gap-4">
            <button
              ...
              onClick={() => props.confirmSave(false)}
            >
              Cancel
            </button>
            <button
              className="btn-green px-2"
              onClick={() => props.confirmSave(true)}
            >
              Confirm
            </button>
          </div>
